...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 22 13:58:53 2018
...
def dense_block(x, blocks, name, attention='cbam_block'):
    """A dense block.
    ...
    # weather add attenton module
    if attention:
      x = attach_attention_module(x,attention)
    return x


def transition_block(x, reduction, name,attention='cbam_block'):
    """A transition block.
    ...
    # weather add attention module
    if attention:
      x = attach_attention_module(x,attention_module=attention)
    return x
...
def DenseNet(blocks,
    ...
    x = layers.Conv2D(64, 7, strides=2, use_bias=False, kernel_regularizer=l2(wd),name='conv1/conv')(x)
...
if __name__ == '__main__':
  model = DenseNet201(include_top=False,weights=None,input_shape=[224,224,3],pooling='avg')
